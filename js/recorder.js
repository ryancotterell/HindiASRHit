...
function microphone_recorder_events()
{
  ...
  case "saved":
    ...
    if(data.saved) {
      $('#upload_status').css({'color': '#0F0'}).text("Your audio file was saved successfully.");
      $('#data_saved').val("true");
    } else {
      $('#upload_status').css({'color': '#F00'}).text("Your audio file could not be saved. Please try again.");
    }
    break;
  ...
  case "save_failed":
    ...
    $('#upload_status').css({'color': '#F00'}).text("Failed: " + errorMessage);
    break;
  ...
  case "save_progress":
    ...
    $('#upload_status').css({'color': '#000'}).text("Progress: " + bytesLoaded + " / " + bytesTotal);
    break;
  }
}
